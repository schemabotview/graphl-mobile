...
// it never dominates a tall box. Children are laid out in the remaining body, so
// they sit clear of the label instead of being bottom-aligned by a coarse row.
const TITLE_BAND = 0.28
const TITLE_CAP = 46

/**
 * Resolve the scene tree into an id -> absolute Box map (top-left origin) for
 * React Flow. Top-level nodes are placed on the scene grid; a node with
 * `children` + `layout` resolves those children INSIDE its own pixel box
 * (recursively), so a container truly measures its contents — NodeMap's
 * `layoutChildren`, flattened to absolute pixels for React Flow.
 */
...
function layoutLevel(
  ...
  // Relative gap/padding (grid units) → pixels, using one cell as the unit so
  // spacing scales with the box.
  ...
  for (const node of nodes) {
    ...
    if (node.children?.length && node.layout) {
      // Reserve a title band for containers (groups are chrome-less).
...
/**
 * Dev-only sanity check on hand-authored cells: warns on nodes that run off
 * their grid or collide. This is the feedback loop the grid otherwise lacks —
 * place cells freely and let the console catch mistakes instead of eyeballing.
 *
 * Validation is per level: each node's `children` are checked against the
 * node's own `layout`, since with real nesting children live in their parent's
 * sub-grid, not the scene grid. Within a level, a `container`/`group` is a
 * backdrop its peers may sit on, and a node may sit inside any larger backdrop
 * term (e.g. kafka's "Consumer Group"). So an overlap is a *genuine* collision
 * only when neither node is a container/group and neither fully contains the
 * other — two peers partially clipping each other.
 */
